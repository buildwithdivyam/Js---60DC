...
menu.addEventListener("click", () => {
    ...
    if (navItems.classList.contains("active")) {
        menu.style.backgroundColor = "rgba(0, 217, 255, 0.3)";
    } else {
...
function getApi(categoryIndex = 3, typeIndex = 1) {
    return `https://v2.jokeapi.dev/joke/${
        category[categoryIndex]
    }?blacklistFlags=${flags.join(",")}&format=json&type=${
        jokePart[typeIndex]
    }&amount=${amount}`;
}
...
jokeContainer.addEventListener("click", (e) => {
    if (e.target.closest(".copy-btn")) {
        const jokeText = e.target
            .closest("li")
            .querySelector(".joke-text").textContent;
        navigator.clipboard.writeText(jokeText);
        e.target.closest(".copy-btn").innerHTML =
